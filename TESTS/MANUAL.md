...
| C.13 | `decoration_style = "rounded_nerdfont"` in a terminal with a Nerd Font-patched font, same signs | Same pill, but capped with real Powerline rounded glyphs; a font without the glyphs shows tofu/blank boxes instead — that is the expected degradation, not a bug |
